...
namespace Client
{
    public partial class MultiPlayForm : Form
    {
        ...
        private bool judge(Stone Player) // 승리 판정 함수
        {
        ...
        private void read()
        {
            while (true)
            {
                ...
                /* 게임 시작 (메시지: [Play]{Stone}) */
                if (message.Contains("[Play]"))
                {
                    refresh();
                    string stone = message.Split(']')[1];
                    if (stone.Contains("Black"))
                    {
                        this.status.Text = "당신의 차례입니다.";
                        nowTurn = true;
                        nowPlayer = Stone.BLACK;
                    }
                    else
                    {
                        this.status.Text = "상대방의 차례입니다.";
                        nowTurn = false;
                        nowPlayer = Stone.WHITE;
                    }
                    playing = true;
                }
                /* 상대방이 나간 경우 (메시지: [Exit]) */
